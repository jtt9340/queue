...
fn usage(program: &str, opts: Options) {
	let desc = format!("Queue \u{2014} a Slack bot to keep track of who is using a 3D \
	printer\nUsage:\n\t{} (-k api-KEY | --key API-KEY>) [-f FILE | --file FILE] [-h | --help]", program);
...
fn main() -> Result<(), slack::error::Error> {
	...
	let program = args.next().expect("Program name was not passed to command line arguments");
